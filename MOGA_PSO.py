...
class MOGA_pso(object):  # 粒子群计算
    def __init__(self, num_cities, num_particles, generations,name1,name2, select_prob=0.8, cross_prob=0.80):  # 种群数量；
        ...
        ## 两个数据包
        coordinates = self.data(name1)  # path_data
        self.timetxt = self.timable(name2)  # time_data
        ...
        self.pbtime_values = np.array(
            [self.tsp_objective_function(p, self.timetxt) for p in self.particles])  # 每个历史最优的距离。
        ##历史最佳位置与时间；
        self.pb_positions = self.particles.copy()  # 初始化个体最佳位置
        self.pb_path = np.array(
            [self.tsp_objective_function(p, self.distance_matrix) for p in self.particles])  # 每个distance历史最优的距离。
        self.pb_time = np.array(
            [self.tsp_objective_function(p, self.timetxt) for p in self.particles])  # 每个time历史最优的距离。
        ##全局的距离。
    ...
    def grid(self, grid_point, n, x_low, x_upper, y_low, y_upper):
        ...
        for j, point in enumerate(grid_point):
            ...
            for i in range(9):
                if (point[0] >= (x_low + x_grid * (i % n)) and point[0] <= (x_low + x_grid * ((i % n) + 1))) and (
                        (point[1] >= (y_low + y_grid * (n - (i // n) - 1))) and point[1] <= (
                        y_low + y_grid * (n - (i // n)))):
    ...
    def tsp(self):
        for _ in range(self.generations):
            ...
            print("self.gb_path=", self.gb_path, "self.gb_time", self.gb_time)
...
# moga_pso_tsp = MOGA_pso(num_cities,num_particles,generation)  # num_cities,num_particles,generations
# moga_pso_tsp.moga_pso()
# plt.figure()
# plt.plot(range(len(moga_pso_tsp.gb_path_lst)), moga_pso_tsp.gb_path_lst)
# plt.plot(range(len(moga_pso_tsp.gb_time_lst)), moga_pso_tsp.gb_time_lst)
# # 设置X轴标签
# plt.xlabel('迭代次数')
# # 设置Y轴标签
# plt.ylabel('路径的总长度')
# # 设置图表标题
# plt.title('moga_pso算法解决tsp问题')
# # 显示网格（可选）
# plt.grid(True)
# # 保存图形为PDF
# plt.savefig('moga_pso.pdf')